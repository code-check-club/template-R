...
You can reference figures or tables using the chunk name, e.g., Figure \@ref(fig:disp-hp-vs) and Table \@ref(tab:mtcars).
...
```{r mtcars}
table1 <- aggregated |>
  kable(caption = "Mean miles per gallon (mpg), displacement (disp) and horsepower (hp) by number of forward gears (gear) and carburetors (carb)", digits = 1) |>
  kable_material()
